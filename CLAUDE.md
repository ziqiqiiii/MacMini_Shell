...
Input flows through these pipeline stages, each in its own numbered source file:
...
**Local binaries:** `init_root()` prepends `$PWD/bin` to `PATH` so programs under `./bin/` are resolved before system directories. The system programs (`backup`, `dcheck`, `dkill`, `dplant`, `dspawn`, `find`, `ld`, `ldr`, `sys`) are standalone C programs compiled into `./bin/`.
...
Files are numbered by pipeline stage: `00_` main, `01_` init/rc/banner, `02_` prompt, `03_` expand, `04_` lexer, `05_` parser, `06_` execute, `07_` pipe, `08_` redirection, `09_` builtins (`09a`–`09l`), `10_` quote, `11_` signal, `12_` free, `13_` utilities. Letter suffixes (e.g. `04a`, `04b`) are overflow files for the same stage.
...
- Error-wrapping functions (`ft_pipe`, `ft_dup2`, `ft_open`, `ft_close`, `ft_fork`) in `13_minishell_utils.c` handle syscall errors and return `EXIT_FAILURE`/`EXIT_SUCCESS`.
